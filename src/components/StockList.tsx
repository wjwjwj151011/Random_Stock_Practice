...
export default function StockList({ stocks, portfolio, selectedStockId, onSelectStock }: StockListProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col h-full" id="stock-list-card">
      <div className="p-3.5 border-b border-slate-100" id="stock-list-header">
        <h3 className="text-sm font-bold text-slate-800 tracking-tight flex items-center justify-between mb-2">
          <span>시장 종목 리스트</span>
          <span className="text-[10px] font-normal text-slate-400">총 {filteredStocks.length}개 종목</span>
        </h3>
        ...
        <div className="relative mb-2.5" id="stock-search-container">
          <input
            ...
            className="w-full text-[11px] pl-7 pr-3 py-1.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-400"
          ...
          {searchTerm && (
            <button 
              ...
              className="absolute right-2.5 top-2 text-slate-400 hover:text-slate-600 font-bold text-[9px]"
            >
              ✕
            </button>
          )}
          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`px-2 py-0.5 text-[9px] font-bold rounded-md whitespace-nowrap transition-all duration-150 ${
                  isCatActive
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-slate-50 max-h-[380px] lg:max-h-none" id="stock-list-container">
        {filteredStocks.map((stock) => {
          ...
          return (
            <button
              ...
              className={`w-full p-2.5 text-left transition-all duration-200 flex items-center justify-between focus:outline-none ${
                isSelected 
                  ? 'bg-slate-50/80 border-l-4 border-slate-800 pl-2' 
                  : 'hover:bg-slate-50/40 pl-2.5 border-l-4 border-transparent'
              }`}
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span 
                  ...
                  className={`text-xs cursor-pointer transition-transform duration-100 active:scale-125 hover:text-amber-500 ${
                    isFav ? 'text-amber-400 font-bold' : 'text-slate-300'
                  }`}
                  title="관심종목 토글"
                >
                  ★
                ...
                <div 
                  className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold font-mono text-xs shadow-xs ${
                    stock.category === 'Tech' ? 'bg-indigo-50 text-indigo-600' :
                    stock.category === 'Bio' ? 'bg-teal-50 text-teal-600' :
                    stock.category === 'Energy' ? 'bg-amber-50 text-amber-600' :
                    stock.category === 'Consumer' ? 'bg-emerald-50 text-emerald-600' :
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    {heldShares > 0 && (
                      <span className="text-[9px] font-bold text-teal-600 bg-teal-50 px-1 py-0.2 rounded" id={`stock-held-badge-${stock.id}`}>
              ...
              <div className="text-right">
                ...
                <div className={`text-[10px] font-semibold flex items-center justify-end gap-0.5 mt-0.5 ${
                  isUp ? 'text-rose-600' : 'text-blue-600'
                }`}>
                  <span>{isUp ? '▲' : '▼'}</span>
