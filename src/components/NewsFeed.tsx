...
export default function NewsFeed({ news, stocks, onSelectStock, onMarkAllAsRead }: NewsFeedProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col h-full" id="news-feed-card">
      <div className="p-3.5 border-b border-slate-100" id="news-feed-header">
        ...
        {/* Filter Tabs */}
        <div className="flex gap-1 bg-slate-50 p-1 rounded-xl" id="news-filter-tabs">
          <button
            onClick={() => setFilter('ALL')}
            className={`flex-1 py-0.5 text-[9px] font-bold rounded-lg transition-all ${
              filter === 'ALL'
                ? 'bg-white text-slate-800 shadow-xs'
      ...
      {viewMode === 'CARD' ? (
        <div className="flex-1 flex flex-col justify-between p-3.5" id="news-card-mode">
          {filteredNews.length > 0 ? (
                    ...
                    >
                      <div
                        ...
                        className={`p-3.5 rounded-2xl transition-all duration-300 border cursor-pointer h-full flex flex-col justify-between ${
                          idx === activeIndex
                            ? 'shadow-md border-slate-200 bg-slate-50/50'
                        ...
                        <div>
                          <div className="flex justify-between items-center mb-2">
                            <span className={`px-1.5 py-0.5 text-[9px] font-bold rounded-lg ${
                              isPositive ? 'bg-rose-50 text-rose-600 border border-rose-100' :
                              isNegative ? 'bg-blue-50 text-blue-600 border border-blue-100' :
                              'bg-slate-100 text-slate-500 border border-slate-200'
                            }`}>
                              {isPositive ? '🔥 호재 (UP)' : isNegative ? '❄️ 악재 (DOWN)' : '📰 일반'}
                            </span>
                            <span className="font-mono text-[9px] text-slate-400 font-bold bg-slate-100/80 px-1.5 py-0.2 rounded-full">
                              {item.time}
                            </span>
                          </div>

                          <h4 className="text-xs font-bold text-slate-800 tracking-tight leading-snug mb-1.5 hover:text-indigo-600 transition-colors">
                            {item.title}
                          </h4>
                        ...
                        <div className="mt-3 pt-2 border-t border-dashed border-slate-200">
                          {impactedStock ? (
                            <div className="flex justify-between items-center text-[10px]">
                              <span className="inline-flex items-center gap-1 text-[9px] font-extrabold text-indigo-600">
                            ...
                            <div className="flex justify-between items-center text-[10px]">
                              <span className="text-[9px] text-slate-400 font-bold">글로벌 거시 경제</span>
                              <span className={`font-mono font-bold ${
              ...
              <div className="flex justify-between items-center mt-3 px-1" id="news-slider-nav">
                <button
                  ...
                  className={`w-7 h-7 rounded-lg border flex items-center justify-center text-[10px] font-bold transition-all ${
                    activeIndex === 0
                      ...
                      : 'text-slate-600 border-slate-200 bg-white hover:bg-slate-50 hover:border-slate-300 active:scale-95'
                  }`}
                  title="이전 속보"
                >
                  ◀
                ...
                <div className="flex flex-col items-center">
                  <span className="text-[10px] font-bold text-slate-500">
                    <span className="text-slate-900 font-extrabold">{activeIndex + 1}</span> / {filteredNews.length}
                  </span>
            ...
            <div className="py-12 text-center text-[11px] text-slate-400 font-medium flex-1 flex items-center justify-center">
              해당 조건의 소식이 아직 없습니다...
            </div>
          )}
        </div>
      ) : (
        <div className="flex-1 overflow-y-auto divide-y divide-slate-50 p-1 max-h-[380px] lg:max-h-none" id="news-list-container">
            ...
            return (
              <div
                ...
                className={`p-2.5 transition-all duration-200 cursor-pointer ${
                  !item.read
                    ? 'bg-amber-50/20 hover:bg-slate-50/40 font-medium border-l-2 border-amber-400 pl-2'
                    : 'hover:bg-slate-50/20 pl-2.5 border-l-2 border-transparent'
                }`}
              ...
              >
                ...
                <h4 className="text-xs font-bold text-slate-800 tracking-tight mb-0.5">
                  {item.title}
                </h4>
                ...
                {impactedStock ? (
                  <div className="flex justify-between items-center" id="news-item-impact">
                    <span className="inline-flex items-center gap-1 text-[9px] font-bold text-indigo-600">
